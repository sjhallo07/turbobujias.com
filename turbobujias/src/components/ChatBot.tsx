      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 sm:inset-auto sm:bottom-6 sm:right-6 z-[600] w-full sm:w-[480px] h-full sm:h-[700px] sm:max-h-[85vh] bg-neutral-900 border-none sm:border sm:border-neutral-800 rounded-none sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden safe-area-padding"
          >
            {/* Header */}
            <div className="pt-12 sm:pt-4 pb-4 px-4 bg-neutral-800/90 flex items-center justify-between border-b border-orange-500/20 sm:rounded-t-3xl backdrop-blur-md sticky top-0 z-20 shadow-lg">
              <div className="flex items-center gap-3">
                <button 
                  onClick={() => setShowSessions(!showSessions)}
                  className={cn(
                    "p-2 bg-neutral-900 border rounded-sm transition-colors",
                    showSessions ? "border-orange-500 text-orange-500" : "border-neutral-700 text-neutral-400 hover:text-white"
                  )}
                  title="Chat Sessions"
                >
                  <History size={18} />
                ...
                <div>
                  <h3 className="text-white font-bold text-sm leading-none mb-1">Turbo Assistant</h3>
                  <div className="flex items-center gap-2">
                    <select 
                      value={selectedModel}
                      onChange={(e) => setSelectedModel(e.target.value)}
                      className="bg-neutral-900 text-[9px] font-black uppercase text-neutral-400 border border-neutral-700 outline-none hover:border-orange-500 transition-colors cursor-pointer px-1"
                    >
              ...
              <div className="flex items-center gap-2">
                ...
                {userMemory && (
                  <div className="group relative">
                    <button className="p-2 text-neutral-400 hover:text-orange-500 transition-colors">
                      <Brain size={18} />
                    </button>
                    <div className="absolute right-0 top-full mt-2 w-64 p-3 bg-neutral-800 border border-neutral-700 shadow-2xl opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-[600]">
                      <h4 className="text-[10px] font-black text-orange-500 uppercase tracking-widest mb-1">Long-term Memory</h4>
                      <p className="text-[11px] text-neutral-200 mb-2 leading-relaxed">{userMemory.summary}</p>
                      <div className="text-[9px] text-neutral-400 font-mono italic">Vehicle: {userMemory.vehicleInfo}</div>
                    </div>
                  </div>
                )}
                <button 
            ...
            <AnimatePresence>
              {showSessions && (
                <motion.div
                  ...
                  animate={{ x: 0 }}
                  exit={{ x: -400 }}
                  className="absolute inset-y-0 left-0 w-64 bg-neutral-900 border-r border-neutral-800 z-[550] flex flex-col shadow-2xl"
                >
                  <div className="p-4 border-b border-neutral-800 flex items-center justify-between bg-neutral-800/50">
                    <h4 className="text-[10px] font-black text-white uppercase tracking-widest">Conversations</h4>
                    <button 
                      onClick={createNewSession}
                      className="p-1.5 bg-orange-500 text-white rounded-sm hover:bg-orange-400 transition-colors"
                    >
                      <Plus size={14} />
                    </button>
                  </div>
                  <div className="flex-1 overflow-y-auto p-2 space-y-1 scrollbar-thin">
                    {sessions.map(s => (
                      <div
                        ...
                        className={cn(
                          "w-full text-left p-2.5 transition-all flex items-center justify-between group border border-transparent cursor-pointer",
                      ...
                      >
                        <div className="flex flex-col gap-1 truncate flex-1">
                          <span className={cn(
                            ...
                            currentSessionId === s.id ? "text-orange-500" : "text-neutral-300"
                          )}>
                            {s.title}
                          </span>
                          {s.lastMessage && (
                            <span className="text-[9px] text-neutral-500 truncate italic">
                              {s.lastMessage}
                            </span>
                          )}
                        </div>
                        <button
                          ...
                          className="opacity-0 group-hover:opacity-100 p-1.5 text-neutral-500 hover:text-red-500 transition-all"
                    ...
                    {sessions.length === 0 && (
                      <div className="p-8 text-center">
                        <p className="text-[10px] text-neutral-600 italic">No previous chats</p>
                      </div>
                    )}
                  </div>
                </motion.div>
            ...
            >
                ...
                >
                  <div className={cn(
                    ...
                    <div className={cn(
                      ...
                      msg.role === 'user' ? "bg-neutral-800" : "bg-orange-500/20"
                    )}>
                      {msg.role === 'user' ? <User size={14} className="text-neutral-400" /> : <Bot size={14} className="text-orange-500" />}
                    </div>
                    <div className={cn(
                      "p-3 rounded-none text-sm leading-relaxed relative group whitespace-pre-wrap",
                      msg.role === 'user' 
                        ? "bg-orange-500 text-white" 
                        : "bg-neutral-800 text-neutral-200 border border-neutral-700 dark"
                    )}>
                      {msg.content}
                      {msg.role === 'bot' && (
                        <button
                          onClick={() => handleCopy(msg.content, msg.id || `${idx}`)}
                          className="absolute bottom-1 right-1 p-1.5 opacity-0 group-hover:opacity-100 bg-neutral-900 border border-neutral-700 text-neutral-400 hover:text-orange-500 transition-all rounded-sm"
                          title="Copy message"
                        >
                          ...
                          {msg.attachments.map((att, attIdx) => (
                            att.type.startsWith('image/') ? (
                              att.data === '__REMOVED_FOR_SIZE__' ? (
                                <div key={`att-${attIdx}`} className="p-4 bg-black/20 border border-neutral-700 flex flex-col items-center justify-center gap-2 text-neutral-500 w-[200px] h-[120px]">
                                  <ImageIcon size={24} />
                                  <span className="text-[10px] text-center">Image removed from history due to size</span>
                                </div>
                              ) : (
                                <img 
                                  key={`att-${attIdx}`}
                                  ...
                                  alt={att.name}
                                  className="max-w-[200px] max-h-[200px] object-contain border border-neutral-700"
                                />
                              )
                            ) : (
                              <div key={`att-${attIdx}`} className="p-2 bg-neutral-900 border border-neutral-700 flex items-center gap-2 text-[10px]">
                                <FileText size={14} />
                                <span>{att.name} {att.data === '__REMOVED_FOR_SIZE__' && '(History only)'}</span>
                              </div>
                            )
                          ))}
                        </div>
                  ...
                  {msg.role === 'bot' && msg.recommendedProduct && !msg.comparisonProducts && (
                    <motion.div 
                      ...
                      className="ml-11 mt-1 p-3 bg-neutral-900 border border-neutral-800 flex flex-col gap-3 dark"
                    >
                      {(() => {
                        ...
                        return (
                          <>
                            <div className="flex items-center gap-3">
                              <img src={product.image} alt={product.name} className="w-12 h-12 object-cover border border-neutral-800" />
                              <div className="flex-1 min-w-0">
                                <h4 className="text-xs font-black text-white truncate uppercase tracking-tighter">{product.name}</h4>
                                <p className="text-[10px] text-orange-500 font-bold">{formatPrice(product.price, selectedCurrency, rates)}</p>
                            ...
                            <div className="flex gap-2">
                              <button 
                                ...
                                className="flex-1 bg-white text-black py-2 text-[10px] font-black uppercase tracking-widest hover:bg-orange-500 hover:text-white transition-all flex items-center justify-center gap-2"
                              >
                  ...
                  {msg.role === 'bot' && msg.comparisonProducts && msg.comparisonProducts.length > 0 && (
                    <motion.div 
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      className="ml-11 mt-1 p-3 bg-neutral-900 border border-neutral-800 grid grid-cols-2 gap-2 dark"
                    >
                      {msg.comparisonProducts.map(product => (
                        <div key={product.id} className="flex flex-col gap-1 border-r border-neutral-800 pr-2 last:border-0 last:pr-0">
                          <img src={product.image} alt={product.name} className="w-16 h-16 object-cover border border-neutral-800" />
                          <h4 className="text-[10px] font-bold text-white truncate uppercase tracking-tighter">{product.name}</h4>
                          <p className="text-[9px] text-orange-500 font-bold">{formatPrice(product.price, selectedCurrency, rates)}</p>
                        </div>
                      ))}
                    </motion.div>
                  )}
              ...
              {isLoading && (
                <div className="flex gap-3 max-w-[85%]">
                  ...
                  <div className="bg-neutral-800 p-3 rounded-none border border-neutral-700">
            ...
            {/* Input */}
            <div className="p-4 pb-8 sm:pb-4 bg-neutral-800/80 backdrop-blur-xl border-t border-neutral-800 sm:rounded-b-3xl">
              {/* Attachments Preview */}
              ...
              <div className="relative flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-2 text-neutral-400 hover:text-orange-500 transition-colors bg-neutral-900 border border-neutral-700"
                  title="Attach images"
                >
                  <ImageIcon size={18} />
                </button>
                ...
                >
                  <input
                    type="text"
                    placeholder="Ask or attach part image..."
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    className="w-full bg-neutral-900 text-white pl-4 pr-12 py-3 rounded-none border border-neutral-700 focus:outline-none focus:border-orange-500 transition-all text-sm"
                    autoFocus
                  />
                  <button
                    type="submit"
